  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const applyEventFilters = (query: ReturnType<typeof supabase.from>) => {
          let filteredQuery = query.eq("status", "approved");
        ...
        const eventResultWithFk = await applyEventFilters(
          supabase.from("events").select(eventSelectWithFk)
        );

        const eventResult = eventResultWithFk.error
          ? await applyEventFilters(supabase.from("events").select(eventSelectLegacy))
          : eventResultWithFk;
