...
import { ADMIN_LISTING_TABLES, buildModerationUpdate, getTableColumns } from "@/lib/admin/data";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ listingId: string }> }) {
  try {
    ...
    const payload = buildModerationUpdate(columns, actor.id, body.action, body.rejectionReason);
